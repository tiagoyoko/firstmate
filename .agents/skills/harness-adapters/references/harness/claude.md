...
Every claude spawn therefore pre-registers the directory its pane starts in before launch, and the dialog does not appear: the task worktree for a ship, scout, or final reviewer, and the home itself for a `--secondmate` spawn, in either seeded shape (a leased worktree or a standalone clone).
