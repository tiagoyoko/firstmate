...
The project's registered delivery posture still comes from this machine's `data/projects.md`, so an unregistered or `local-only` project is refused rather than provisioned.
