...
It classifies recognized Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, Muse, Rovo, Oh My Pi (`omp`), and AGY process identities as `alive`, common shells as `dead`, an authoritatively absent window as `missing`, unreadable state as `unreadable`, and every other process as `ambiguous`.
...
The foreground probe also reads argv[0] so an exact harness install-path component can carry the verdict when the other fields expose a rewritten process name, and it reads the command line for narrowly verified interpreter-script identities.
...
omp accepts either the exact `omp` process identity or a `bun` process whose script operand has the exact basename `omp`; `ompd`, `comp`, an `omp` directory component, and a later argument named `omp` remain ambiguous.
