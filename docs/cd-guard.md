...
It is a silent no-op (exit 0, no output) everywhere else, so it never interferes with a ship, scout, or final reviewer that legitimately works inside its own project or firstmate task worktree.
...
A ship, scout, or final-reviewer task worktree - the shape `bin/fm-spawn.sh` always hands out - is a linked git worktree where the two differ, so the guard is inert there.
...
Secondmate child ship, scout, and final-reviewer worktrees are likewise inert under the linked-worktree test.
