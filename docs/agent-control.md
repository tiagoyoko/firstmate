...
`relaunch` is the only verb that changes durable records, so it runs as a transaction with a journal at `state/<id>.control-relaunch`, the prior record preserved beside it, and a ship, scout, or final reviewer's prior instructions preserved when a progress note is appended.
...
1. **Resolve the profile.**
   ...
   A ship, scout, or final reviewer keeps the harness already recorded for it, because that harness comes from firstmate's dispatch-profile judgment at intake and must not be silently re-read from configuration.
   A recorded raw-command basename that differs from its resolved adapter cannot reproduce the command actually running, so relaunch refuses before the checkpoint unless the caller passes an explicit `--harness` to choose the replacement runtime deliberately.
   A harness change resets model and effort unless they are named too, because a model chosen for one adapter does not transfer to another.
2. **Safe checkpoint.**
   The recorded worktree must exist and be a worktree root; its head and dirty state are recorded.
   A final reviewer additionally requires a clean worktree whose current HEAD exactly matches its immutable spawn-time `review_head`, so a replacement cannot review a changed snapshot.
   For a `kind=secondmate` task, the home's identity marker must match and its child records must be readable, so a relaunch can never strand child work behind an unreadable home.
   A secondmate's own crewmates run in their own endpoints and outlive its relaunch; the relaunched secondmate reconciles them from its home's durable records at startup.
3. **Record the note.**
   A ship, scout, or final-reviewer relaunch requires `--note`, because the replacement inherits the local copy but none of the conversation; the note is appended to the instructions it reads.
   A secondmate relaunch does not require one and never rewrites its standing charter.
4. **Stop the old agent** through the `exit` verb, with its postcondition.
   If a completed final review is being relaunched, the control plane appends a review-reopened status after the old agent stops so its earlier terminal verdict cannot remain current.
...
- An adapter that is not verified for this task's kind is refused **before** the running agent is stopped, not after.
  Muse is a worktree-worker adapter only, so relaunching a secondmate onto it refuses while its agent is still up rather than leaving that secondmate with no agent when the launch owner refuses.
- A final-reviewer relaunch refuses a dirty worktree or any HEAD different from its recorded initial review snapshot before stopping the current agent.
- Rovo is refused for final reviewers because it cannot load the required `reasoning-critique` skill outside the reviewed worktree.
